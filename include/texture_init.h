/*
** EPITECH PROJECT, 2024
** texture
** File description:
** init
*/

#ifndef TEXTURE_INIT_H
    #define TEXTURE_INIT_H

    #include <stddef.h>

static const char *INIT_TEXTURE_TAB[] = {
    "assets/icons/basic.png",
    "assets/icons/file.png",
    "assets/icons/save.png",
    "assets/icons/import.png",
    "assets/icons/new_file.png",
    "assets/icons/edition.png",
    "assets/icons/help.png",
    "assets/icons/about.png",
    "assets/icons/pencil.png",
    "assets/icons/eraser.png",
    "assets/icons/brush.png",
    "assets/icons/pipette.png",
    "assets/icons/bucket.png",
    "assets/icons/size.png",
    "assets/size/litle.png",
    "assets/size/litle-big.png",
    "assets/size/medium.png",
    "assets/size/big-litle.png",
    "assets/size/big.png",
    "assets/size/enormous.png",
    "assets/icons/clear.png",
    "assets/icons/flip_horizontal.png",
    "assets/icons/flip_vertical.png",
    "assets/colors/red.png",
    "assets/colors/black.png",
    "assets/colors/blue.png",
    "assets/colors/green.png",
    "assets/colors/orange.png",
    "assets/colors/cyan.png",
    "assets/colors/purple.png",
    "assets/colors/yellow.png",
    "assets/colors/brown.png",
    "assets/colors/pink.png",
    "assets/colors/beige.png",
    "assets/colors/white.png",
    "assets/colors/grey.png",
    "assets/colors/fushia.png",
    "assets/colors/dark_green.png",
    "assets/colors/dark_cyan.png",
    "assets/colors/dark_yellow.png",
    "assets/colors/light_beige.png",
};

static const size_t TEXTURE_TAB_S = sizeof(INIT_TEXTURE_TAB) / sizeof(char *);

#endif
