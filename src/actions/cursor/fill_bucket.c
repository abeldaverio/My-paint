...
void fill_bucket(cursor_t *cursor, board_t *board,
    sfVector2i *mouse, sfEvent *)
{
    sfColor colors[2] = {sfImage_getPixel(board->image,
    ...
    if (mouse->x < LENGTH - 2 && !is_same_color(colors[0], colors[1]))
        fill_the_board(colors, board->image, &(sfVector2i){mouse->x + 1,
            mouse->y - BOARD_ESPACEMENT}, &(sfVector2i){-1, 1});
    if (mouse->y > BOARD_ESPACEMENT && !is_same_color(colors[0], colors[1]))
        fill_the_board(colors, board->image, &(sfVector2i){mouse->x,
            mouse->y - BOARD_ESPACEMENT - 1}, &(sfVector2i){1, -1});
    if (mouse->x < LENGTH - 2 && !is_same_color(colors[0], colors[1]))
        fill_the_board(colors, board->image, &(sfVector2i){mouse->x + 2,
            mouse->y - BOARD_ESPACEMENT}, &(sfVector2i){1, 1});
